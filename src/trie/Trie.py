...
"""
A Trie, or prefix tree, is an ordered-tree structure used to store dynamic information that can be searched for.
It's a unique type of tree because unlike a binary tree, no node in the tree stores the key that you're searching for - instead,
the key itself is distributed.

Using a Trie is better for a large collection of words then searching a list of words in python, which takes 0(N)
"""
